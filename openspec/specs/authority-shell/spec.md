...
The site's authority-building content surface — methodology, about, contact, election
calendar — and the social content plan that drives audience to it before real
candidate/poll data exists (`BL-06`).
...
Every published page SHALL carry the same footer contact block (WhatsApp link, links to methodology/about).

#### Scenario: Footer consistent across all pages
- **WHEN** any published static page is loaded
- **THEN** it contains the same WhatsApp link and links to `/metodologia.html` and `/quienes-somos.html`
