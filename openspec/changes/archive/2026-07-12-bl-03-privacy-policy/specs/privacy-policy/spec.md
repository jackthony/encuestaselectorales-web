...
- **THEN** it contains an explicit statement that no personal data is collected as of publish date, phrased so it can be updated later (per `design.md` decision 1) without rewriting the rest of the page
...
- **THEN** each item states the binding commitment (e.g. "nunca guardamos tu IP sin hashear") rather than locking in an exact algorithm as the promise itself (per `design.md` decision/risk on over-promising implementation specifics)
...
- **THEN** it references `BL-03` as a gate that includes re-review, not just prior existence, consistent with `design.md` decision 2
