...
4. **Prove once, replicate by data.** `BL-06`-`BL-13` prove the pattern on 1 pilot district. `BL-27` repeats it across the other 42 with JSON, not new engineering. Don't build for 43 districts before 1 works end-to-end.
5. **No backend before its gates.** `BL-24` (own-poll backend) does not activate without `BL-03` (privacy policy) and `BL-25` (anti-abuse) already shipped. Static site until then.
...
- Numbering IS execution order (BL-01 first → BL-30 last). Pick the first `not-started` item whose dependencies are all `done`. Don't skip ahead, don't batch multiple items in one session unless told to.
