...
Each active survey card SHALL show the territorial level next to the place name, such as `Distrito Miraflores`, `Provincia Callao`, or `Región Lima`, so same-named places remain distinguishable in the public feed.
...
- **THEN** "Encuestas Web Activas" renders a card linking to the correct district or territory page, sourced from the `encuestas` table rather than `data/encuesta.json`
