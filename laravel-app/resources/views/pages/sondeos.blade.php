            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                <div class="lg:col-span-8">
                    <div class="flex justify-between items-baseline mb-8 border-b border-brand-border pb-4">
                        <h2 class="text-3xl font-serif font-bold text-brand-blue">Encuestas Web Activas</h2>
                    ...
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach ($activeRounds as $round)
                                @php $district = findDistritoById((string) ($round['distrito_id'] ?? '')); @endphp
                                <article class="bg-brand-card border border-brand-border rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col">
                                    <div class="flex justify-between items-start mb-3 gap-4">
                                        <div>
                                            <h3 class="font-serif font-bold text-2xl text-brand-blue leading-tight">{{ surveyScopeLabel($round, $district) }}</h3>
                                    ...
                                    <div class="mt-auto pt-5 border-t border-brand-border flex justify-between items-center">
                                        <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Voto web activo</span>
                                        <a href="{{ surveyTargetUrl($round) }}" class="text-[13px] font-bold text-brand-blue hover:text-brand-green flex items-center gap-1.5 group transition-colors">
                                            Ir al sondeo
                            ...
                            @else
                                <div class="space-y-3">
                                    @foreach (array_slice($fieldStudies, 0, 5) as $study)
                                        @php $pollster = findEncuestadoraById((string) ($study['encuestadoraId'] ?? '')); @endphp
                                        <a href="{{ url('/encuesta.php?id=' . rawurlencode((string) ($study['id'] ?? ''))) }}" class="block rounded-xl border border-brand-border bg-white p-4 hover:border-brand-blue/30 transition-colors">
                                            <div class="text-[10px] font-bold text-brand-muted uppercase tracking-widest mb-1">{{ $pollster['nombre'] ?? 'Encuestadora' }}</div>
