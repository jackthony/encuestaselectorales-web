        ...
        <div class="space-y-6">
            ...
            <div class="bg-brand-card border border-brand-border rounded-2xl p-4 md:p-5 shadow-sm">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <label class="block">
                        <span class="block text-[10px] font-bold uppercase tracking-widest text-brand-muted mb-2">Estado</span>
                        <select id="home-voting-status" class="w-full rounded-xl border border-brand-border bg-white px-4 py-3 text-sm font-semibold text-brand-blue focus:border-brand-blue focus:outline-none">
                ...
                <p class="mt-3 text-xs text-brand-muted">
                    La jerarquía usa región, provincia y distrito; el estado distingue activas e inactivas; la ronda filtra entre Ronda 1 y Ronda 2.
                </p>
            </div>
                ...
                <div id="home-voting-list" class="space-y-3">
                    @foreach ($rondasAbiertas as $ronda)
                        @php
                            ...
                            $readinessState = (string) ($ronda['readiness_state'] ?? 'inactive');
                            $stateLabel = $readinessState === 'active' ? 'Activa' : 'Inactiva';
                            $stateClasses = $readinessState === 'active'
                                ? 'bg-[#e6f8f0] text-brand-greenText border-[#15ba75]/30'
                            ...
                            <div class="grid grid-cols-1 gap-4 px-5 py-5 md:grid-cols-12 md:items-center">
                                <div class="md:col-span-5 min-w-0">
                                    <div class="flex flex-wrap items-center gap-2 mb-2">
                                        ...
                                        <span class="text-[10px] uppercase tracking-widest font-bold px-2.5 py-1 rounded-full border {{ $isSelected ? 'bg-white/15 text-white border-white/15' : $stateClasses }}">
                                            {{ $stateLabel }}
                                        </span>
                                        <span class="text-[10px] uppercase tracking-widest font-bold px-2.5 py-1 rounded-full {{ $isSelected ? 'bg-white/15 text-white' : 'bg-brand-surface text-brand-muted border border-brand-border' }}">
                                            Ronda {{ $roundNumber }}
