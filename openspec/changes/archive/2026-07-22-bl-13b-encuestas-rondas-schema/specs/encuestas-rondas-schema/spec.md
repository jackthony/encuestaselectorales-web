...
The system SHALL also store a `nivel` discriminator for each round with one of `distrito`, `provincia`, or `region`, so same-named places can be separated in the public UI and in the underlying queries.
...
#### Scenario: Same slug, different territorial level
- **WHEN** a slug exists in more than one territorial level, such as `callao` as a district and `callao` as a region
- **THEN** the public site keeps those rounds separate by their `nivel`, and each page shows the level next to the place name
...
The creation path SHALL require or accept an explicit territorial level, so operators can create district-only, province-only, or region-only rounds without inventing new naming schemes.
