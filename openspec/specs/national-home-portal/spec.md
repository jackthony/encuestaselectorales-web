...
## Purpose
`index.php` is the national home portal for EncuestasElectorales.pe — first page a visitor sees, scoped to all of Peru rather than any single city, with a district search and two real-data hub columns (active online rounds, published field studies).
...
The hero search input SHALL filter `data/distrito.json` by name and link each result to `distrito.php?slug=<id>`, working for every entry in the catalog regardless of its size.

The shared header's own search control (`partials/header.php`) is a separate, unwired icon-only button — it predates this capability and reusing it verbatim took priority over wiring it too. Wiring it is a follow-up, not a requirement of this capability today.

#### Scenario: A search term matches and links correctly
- **WHEN** a user types a district's name (or a matching substring) into the hero search input
