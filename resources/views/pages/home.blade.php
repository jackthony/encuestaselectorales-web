                ...
                <div class="space-y-4">
                    ...
                    @else
                        @foreach ($rondasAbiertas as $ronda)
                            <a href="{{ $ronda['target_url'] }}" class="block bg-brand-card rounded-2xl p-6 border border-brand-border hover:shadow-lg hover:border-brand-blue/30 transition-all">
                                <div class="flex justify-between items-start mb-3">
                                    <div>
                                        <h3 class="font-bold text-xl text-brand-blue">{{ $ronda['scope_label'] }}</h3>
                                        <p class="text-xs text-brand-muted mt-1">{{ $ronda['titulo'] }}</p>
                                    </div>
                                    @if ($ronda['readiness_state'] === 'active')
                                    ...
                                    @else
                                        <span class="bg-amber-50 text-amber-800 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-widest border border-amber-200">Sin candidatos verificados</span>
                                    @endif
                                </div>
                            </a>
                        @endforeach
                    ...
                    @else
                        @foreach ($encuestas as $estudio)
                            <a href="encuesta.php?id={{ $estudio['id'] }}" class="block bg-brand-card rounded-2xl p-6 border border-brand-border hover:shadow-md transition-shadow">
                                <div class="text-[10px] font-bold text-brand-muted uppercase tracking-widest mb-1">{{ $estudio['encuestadora_nombre'] ?? '' }}</div>
