...
- [x] 1.4 Verify: a throwaway script connects successfully from both a local `php -S` run and the deployed site.
...
## 7. Green
- [x] 7.1 Create a `prueba` round via the script; confirm it does NOT appear on `index.php` or `distrito.php`.
- [x] 7.2 Flip it to `producción` with a window covering now; confirm it DOES appear in both places.
- [x] 7.3 `scripts/check-refactor.php` and all `validate-*.js` still green.
