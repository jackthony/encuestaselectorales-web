@php
    require_once dirname(base_path()) . '/includes/helpers.php';
    ...
    $catalog = require dirname(base_path()) . '/includes/data.php';
    $partidos = is_array($catalog) ? ($catalog['partidos'] ?? []) : [];
@endphp
<meta charset="utf-8">
...
@foreach ($partidos as $party)
                        {{ strtolower((string) ($party['siglas'] ?? '')) }}: '{{ $party['color'] ?? '#6b7280' }}',
@endforeach
                    }
                },
                boxShadow: {
