...
### Requirement: Retrofitted page passes accessibility under new tokens
`politica-editorial.html` (the existing `BL-02` page) SHALL be re-verified against the same accessibility checklist `BL-02` used, under the new color tokens — not assumed to still pass unchanged.

#### Scenario: Contrast still meets WCAG AA after retrofit
- **WHEN** text/background contrast ratios are computed for primary text, muted text, and accent-colored text against their respective surfaces on the retrofitted page
- **THEN** every ratio is ≥ 4.5:1 (WCAG AA for normal text)

#### Scenario: No regression in heading hierarchy or landmarks
- **WHEN** the retrofitted page's DOM is inspected
- **THEN** heading order is still H1→H2→H2 (no skips), and `main`/`footer`/`section#firewall` landmarks are still present (unchanged from `BL-02`, verified not broken by the CSS swap)
