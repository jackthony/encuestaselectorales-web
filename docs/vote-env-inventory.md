...
**Estado actual**: 0/50 distritos con bounds configurados. No existe ningún archivo en el repo con lat/lng por distrito todavía.

**Comportamiento si falta en prod**: `ConfiguredGeographicValidator::contains()` ya no bloquea por ausencia de bounds; el flujo sigue con los demás controles de voto y la precisión GPS. Si más adelante queremos volver a un geofence estricto, bastará con poblar este JSON.
...
- Sin bounds → el voto sigue, pero sin geofence por distrito.
...
- Fuente de los polígonos/bounding boxes por distrito, sólo si decidimos reactivar el geofence estricto (¿shapefile INEI, Geoportal, algo ya usado en `scope.blade.php` para el mapa?).
